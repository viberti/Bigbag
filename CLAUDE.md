...
- **Sync:** `GET /api/produto/base-local-fichas` (incremental por cursor de `ean` + `versao` p/ resync no rebuild). `baseLocal.js` → `sincronizarFichasBulk` (cursor/versão próprios) escreve na store `fichas`. `Shell` pré-carrega no mount.
- **Local-first (v2):** `aoCodigo` (scan) consulta `fichaLocal` ANTES do servidor → resolve instantâneo/offline nos hits; a `Ficha` renderiza a base local enquanto o `/info` resolve (não sobrepõe se o servidor já respondeu).
- **Telemetria hit/miss:** `POST /api/produto/local-hit` → `base_local_evento` (resolvido-no-telefone vs servidor). `registarHitLocal` no scan. Relatório: `scripts/taxa_base_local.mjs` (taxa de hit + **top misses cruzados com off_full** = o que adicionar a seguir) + **aba `/admin` "Base local"** (`GET /api/admin/base-local`: taxa, tamanho por origem, por contexto/dia, top misses).
- **CRESCE COM O USO (dono, 2026-06-17):** todo o EAN resolvido FORA do cliente (um MISS, buscado no servidor) entra na `base_local` PARTILHADA com nutrição/ingredientes → vira HIT na próxima sync de TODOS os telefones. `consultarOuGuardar` faz `upsertBaseLocal` (origem `uso`) ao gravar a ficha fundida. Por isso a sync é por **`seq` AUTO_INCREMENT** (migr. 068), não por `ean` — o cursor por ean não descia uma inserção viva com ean menor. **Rebuild** (`build_base_local.mjs`) é **idempotente por upsert (sem TRUNCATE)** → preserva as linhas `uso`. Limitação conhecida: o ENRIQUECER de uma linha já existente (ganhar nutrição) não re-sincroniza até um rebuild (o `seq` não muda no UPDATE).
- **Afinação conhecida:** nomes Mercadona-ES/estrangeiros ficam na língua original quando `nome_pt` é null (a tradução lazy no scan corrige no uso online); levar o `nome_pt` persistido à base é melhoria futura.
