...
**Infra FECHADA:** `https://bigbag.hal9klabs.com` (Apache+Let's Encrypt), `bigbag-backend.service` (systemd, porta 4200). BD `app_bigbag`, **migrações até 040** (013-032 base; 033 nutricao_confirmada; 034 lista_compras; 035 conteúdo da embalagem; 036 marca_origem; 037 verificacao_nome; 038 off_produto; 039 chave larga em produto_analise; 040 catalogo nome_pt). Migrações novas: aplicar com `mysql … < ficheiro` no servidor.
...
- **Normalização v2 FASE A** (migrações 035-036) — conteúdo da embalagem→ppb, motor de busca interno no catálogo (`buscarCatalogo`), dicionário de abreviaturas minado, marca determinística pré-LLM (`marca_origem`), IVA por voto maioritário, vocabulário único de facetas. Ver `Analise_Fontes_Normalizacao.md` (Fase A) e `Normalizacao.md`.
...
- **Fontes de catálogo ~60k** (`catalogo_produto`) — Auchan/Continente (scrape, c/ EAN), Pingo Doce/Lidl (sem EAN), **Mercadona** (API JSON ES), **lidl-fr** (lista QCE FR), **mercadona-off** (own-brand do OFF, nomes PT). + `off_produto` (dump do OFF, ~27k, **`consultarOFF` local-first**).
- **Lista de compras partilhada** (3 fases, migração 034) — servidor é fonte de verdade, sync por polling, cores por membro, reconciliação com o talão, listas individuais, voz→lista.
...
- **Backlog:** lista de compras **preditiva** ("para X dias→N unidades", algoritmo da tese MSc); **agregado familiar** (consumo partilhado); Fase B da normalização (categoria enum, consulta IDF, NIF comprador+pagamento); Auditoria UX (`design/review/Auditoria_UX_PLANO.md`); colapsar itens iguais com nº de unidades diferente (aba Itens); i18n leftovers (PerfilSheet, ResultadoIdent, aria-labels); `/identificar` sem transação.
