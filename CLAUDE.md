...
**Ferramenta "peso pela imagem"** (`ingest/pesoImagem.js`, migração 051): ~30k produtos de catálogo (Continente/PD) têm imagem mas formato "1un" (peso não está no título; página é JS+anti-bot, mas o CDN da imagem é público). LAZY em fundo a partir da lista (item sem peso → job 1×/EAN); cascata imagem-catálogo(1000px)→OFF(full)→busca-imagem-CSE(403, salta)→foto manual; cirúrgico (só peso), plausibilidade 5g–15kg, tentado-uma-vez (`peso_img_em`), custo `peso-imagem`, `versaoPesoImg` no ETag. Taxa medida ~1/3 (o resto não tem peso legível na frente — fica marcado, sem repetir custo).
