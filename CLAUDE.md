...
**Infra FECHADA:** Apache+Let's Encrypt, systemd porta 4200. BD `app_bigbag`, **migrações até 059** (059 = `produto_busca`, índice FULLTEXT de busca; 058 = `catalogo_produto.product_type`; lista no `Schema_e_Funcoes_ToolUse.md §1d`). Migrações novas: `mysql … < ficheiro` (ou via node) no servidor (aditivas por regra). *(Nota: `off_full` foi criada por script, não por migração numerada.)*
