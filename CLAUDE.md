...
- **Resolução por NOME de EAN desconhecido** (`normaliza/resolverPorNome.js`, `acharPorNomeMarca`, PROVADO mas POR LIGAR): EAN não encontrado → foto → VLM lê nome+marca → busca o **mesmo produto sob OUTRO EAN** no `off_full` (FULLTEXT `ft_nome`, marca=gate forte, tamanho desempata) → adota nutrição+**imagem**. A construir: cruzar com o **match-por-imagem** (`matchImagem.js`, a foto valida) + fusão de confiança + UI de confirmação + vetorizar a imagem adotada. Visão: texto acha, imagem valida, índice cresce com o uso.
