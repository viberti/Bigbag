...
- **Gémeo:** gate de precisão da sugestão `nomeCondizGemeo` (só propõe se nome/termos condisserem, não só a marca); `adotarNomeRef` + imagem do `/info` lêem off_full **E** catálogo (gémeo só-BR adota-se); o `gemeo` do `/identificar` ainda é descartado no frontend (Fase 2 pendente).
