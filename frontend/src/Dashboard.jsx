// Dashboard de GESTÃO (/dash) — layout BI com MENU LATERAL. Cada secção é a sua página:
// Visão global (KPIs) · Fontes (tabela) · Países (cards) · Custos de IA. Reusa a auth/adminApi.
...
const nomePais = (p) => (p === '—' ? 'Outras' : p === 'Global' ? 'Global (OFF)' : p);
...
const NAV = [['global', 'Visão global', '🌐'], ['fontes', 'Fontes', '🗂️'], ['paises', 'Países', '🌍'], ['custos', 'Custos de IA', '💸']];
const TITULOS = { global: 'Visão global', fontes: 'Fontes de dados', paises: 'Países', custos: 'Custos de IA' };
...
function DashApp() {
  ...
  const [pagina, setPagina] = useState('global');
  const [pais, setPais] = useState(''); // filtro partilhado (clicar num país → Fontes filtradas)
  const irFontesPais = (p) => { setPais(p); setPagina('fontes'); };

  return (
    <div className="dash-shell">
      <aside className="dash-side">
        <div className="dash-brand">📊 <b>BigBag</b><span>dashboard</span></div>
        <nav className="dash-nav">
          {NAV.map(([id, l, ic]) => (
            <button key={id} className={pagina === id ? 'on' : ''} onClick={() => setPagina(id)}><span className="dn-ic">{ic}</span>{l}</button>
          ))}
        </nav>
        <div className="dash-side-foot">
          <button className="dash-refresh2" onClick={() => carregar(true)} disabled={d == null && !erro}>↻ recalcular dados</button>
          <a href="/">← voltar à app</a>
          {d?.gerado_em && <span className="dash-ts">dados de {new Date(d.gerado_em).toLocaleString('pt-PT')}</span>}
        </div>
      </aside>

      <main className="dash-main">
        <div className="dash-main-h"><h1>{TITULOS[pagina]}</h1></div>
        {pagina === 'custos' ? <PgCustos />
          : erro ? <p className="adm-vazio">Falha a carregar o dashboard.</p>
            : d == null ? <p className="adm-vazio">a calcular o panorama…</p>
              : pagina === 'global' ? <PgGlobal d={d} />
                : pagina === 'paises' ? <PgPaises d={d} onPais={irFontesPais} />
                  : <PgFontes d={d} pais={pais} setPais={setPais} />}
      </main>
    </div>
  );
}
...
function PgGlobal({ d }) {
  const g = d.global || {};
  const sis = d.fontes.reduce((a, x) => ({ produtos: a.produtos + x.produtos, fotos: a.fotos + x.fotos, nut: a.nut + x.com_nutricao }), { produtos: 0, fotos: 0, nut: 0 });
  return (
    <>
      <div className="dash-kpis">
        ...
        <Kpi n={fmt(g.ean_com_nutricao)} l="EANs com nutrição (OFF)" sub={`+ ${fmt(g.ean_com_nutricao_catalogo)} no catálogo`} />
        <Kpi n={fmt(g.ean_catalogo)} l="EANs no catálogo (retalho)" />
      </div>
      <h3 className="dash-h3">Totais do sistema</h3>
      <div className="dash-kpis">
        <Kpi n={fmt(d.fontes.length)} l="Fontes integradas" />
        <Kpi n={fmt(sis.produtos)} l="Produtos (todas as fontes)" />
        <Kpi n={fmt(sis.fotos)} l="Fotografias" />
        <Kpi n={fmt(sis.nut)} l="Registos com nutrição" />
      </div>
    </>
  );
}
...
/* ── Países ───────────────────────────────────────────────────────────────── */
function PgPaises({ d, onPais }) {
  const maxE = Math.max(1, ...d.paises.map((p) => p.eans));
  return (
    <>
      <p className="dash-sub">Métricas por país. Clica num país para ver as suas fontes.</p>
      <div className="dash-paises">
        {d.paises.map((p) => (
          <button key={p.pais} className="dash-pais" onClick={() => onPais(p.pais)}>
            <div className="dp-top"><span className="dp-flag">{BANDEIRA[p.pais] || '·'}</span><span className="dp-nome">{nomePais(p.pais)}</span></div>
            <div className="dp-num"><b>{fmt(p.n_fontes)}</b> fontes ativas</div>
            <div className="dp-num"><b>{fmt(p.eans)}</b> EANs</div>
            <div className="dp-bar"><i style={{ width: `${Math.round(100 * (p.eans / maxE))}%` }} /></div>
          </button>
        ))}
      </div>
    </>
...
/* ── Fontes (tabela ordenável/filtrável/pesquisável + CSV) ────────────────── */
...
function PgFontes({ d, pais, setPais }) {
  ...
  }, [d.fontes, pais, busca, ord]);

  const resumo = filtradas.reduce((a, x) => ({ produtos: a.produtos + x.produtos, eans: a.eans + x.eans, fotos: a.fotos + x.fotos, nut: a.nut + x.com_nutricao, ing: a.ing + x.com_ingredientes }), { produtos: 0, eans: 0, fotos: 0, nut: 0, ing: 0 });
  ...
  function exportarCSV() {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'bigbag-fontes.csv'; a.click(); URL.revokeObjectURL(a.href);
  }

  return (
    <>
      <div className="dash-toolbar">
        <select className="dash-sel" value={pais} onChange={(e) => setPais(e.target.value)}>
          <option value="">Todos os países</option>
          {paisesOpts.map((p) => <option key={p} value={p}>{BANDEIRA[p] || ''} {nomePais(p)}</option>)}
        </select>
        <input className="dash-busca" placeholder="pesquisar fonte / plataforma…" value={busca} onChange={(e) => setBusca(e.target.value)} />
        <span className="dash-count">{filtradas.length} fontes</span>
      ...
      <div className="dash-tabela-wrap">
        <table className="dash-tabela">
          ...
          <tbody>
            {filtradas.map((x) => (
              <tr key={x.fonte}>
                <td className="dash-fn">{x.fonte}</td>
          ...
          <tfoot><tr className="dash-foot">
            <td colSpan="3">Total ({pais ? nomePais(pais) : 'todos'})</td>
...
function PgCustos() {
  ...
  return (
    <>
      <div className="dash-toolbar">
        <span className="dash-tb-l">Período:</span>
        <div className="dash-periodo">{[7, 30, 365].map((dd) => <button key={dd} className={dias === dd ? 'on' : ''} onClick={() => setDias(dd)}>{dd === 365 ? 'ano' : dd + ' dias'}</button>)}</div>
      </div>
      {c == null ? <p className="adm-vazio">a carregar custos…</p> : c === false ? <p className="adm-vazio">Falha a carregar custos.</p> : (
        <>
          <div className="dash-kpis">
            <Kpi n={usd(c.total?.usd)} l={`Gasto (${dias} dias)`} />
            <Kpi n={fmt(c.total?.chamadas)} l="Chamadas de IA" />
            <Kpi n={usd(c.geral?.usd)} l="Gasto total (histórico)" />
          </div>
          <div className="dash-custos-grid">
            <div>
              <h3 className="dash-h3">Por operação</h3>
              <table className="dash-tabela">
                ...
                <tbody>{(c.por_contexto || []).map((x) => (
                  <tr key={x.contexto}><td className="dash-fn">{x.contexto}</td><td className="num">{fmt(x.chamadas)}</td><td className="num dash-usd">{usd(x.usd)}</td><td className="num"><small>{usd(x.media)}</small></td></tr>
                ))}</tbody>
              </table>
              ...
              <table className="dash-tabela">
                ...
                <tbody>{(c.por_modelo || []).map((x) => (
                  <tr key={x.modelo || '?'}><td className="dash-fn">{x.modelo || '—'}</td><td className="num">{fmt(x.chamadas)}</td><td className="num dash-usd">{usd(x.usd)}</td></tr>
                ))}</tbody>
              </table>
            </div>
            <div>
