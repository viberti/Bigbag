...
(function(){
  ...
  const nav=(on)=>{const cur=typeof on==='number'?["home","lista","despensa","perfil"][on]:on;
    ...
    const tab=([ic,lb,go])=>`<button class="nb ${cur===go?'on':''}" data-go="${go}"><span class="ni">${I(ic,{size:23,stroke:2})}</span>${lb}</button>`;
    return `<div class="cnav">${left.map(tab).join("")}
      <button class="nb-scan" data-go="scanner" title="Consultar produto">${I('scan',{size:28,stroke:2.4,color:'#5a4410'})}</button>
      ${right.map(tab).join("")}</div>`;};
  ...
  S.home=()=>`${stbar}${motif}${ctop('Olá, Sue','vamos às compras?',{av:1})}
    <div class="scrollarea">
      ...
      <div class="quick">
        ...
        <button class="round-act" data-go="notas"><span class="circ" style="color:#3b86c4">${I('chart',{size:24,stroke:2.4})}</span><b>Gastos</b></button>
      </div>
