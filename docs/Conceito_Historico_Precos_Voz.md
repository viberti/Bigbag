...
7. **Decisões fechadas na sessão de scans 2026-06-12** (detalhe no CLAUDE.md e em `Exploracao_Marca_e_Fontes.md`):
   - **Resolução do nome de um EAN é PT-first:** nome de loja PT do nosso catálogo (mesmo EAN) > OFF traduzido por LLM (síncrono no scan→lista via `?pt=1`; em fundo nas fichas; auto-cura da base). Caso-prova: Barilla Penne — "Massa Penne Rigate Barilla" (Continente) vs "Penne Rigate No. 73 Durum Wheat…" (OFF).
   - **Classificação tem DUAS lentes:** de **loja** (corredor, `sku.grupo` — massa/arroz/cereais em *mercearia*, padaria=só pão; serve comprar/explorar) e de **lista/consumidor** ("o que a coisa É": Massa, Pão, Cereais, Conservas + Mercearia residual; cabeçalhos da lista). Os genéricos a cortar do nome no display estão **associados à categoria** ("pasta" corta-se em Massa, fica em "Pasta de Dentes").
   - **Despensa = inventário por SCAN** (049), não derivado de compras.
   - **Preço de catálogo na lista:** item scaneado nunca comprado mostra o **menor** preço de catálogo entre lojas como "~€x · online" (referência, nunca critério/facto).
   - **EAN desconhecido nunca se descarta:** regista-se `pendente`; o scanner oferece "Identificar por foto" (VLM grava a ficha sob o EAN).
...
**Resolvedor ÚNICO da ficha por EAN — DECISÃO FECHADA (2026-06-13).** Pergunta do dono: "não verificamos todas as nossas fontes desse EAN para produzir a melhor informação possível?" — não verificávamos; a gravação era "first-wins com remendos" espalhada por 5 módulos. Agora a ficha gravada (`produto_ean`) é a **FUSÃO campo-a-campo de TODAS as fontes locais** (linhas de catálogo do EAN + dump OFF + OFF live gravado + VLM), com a tabela de prioridades **num só sítio** (`normaliza/fichaEan.js`) e proveniência por campo em `produto_ean.fusao` (052). Decisões do dono: (1) **nutrição invertida** — catálogo-oficial > OFF (crowdsourced) > VLM-plausível; (2) **divergências só se registam** (sem worklist); (3) **backfill aplica e regista** o diff para revisão posterior; (4) **nome**: limpar marca+formato dos candidatos primeiro — a maioria colapsa — e só o resíduo se arbitra (consenso de tokens ≥2 fontes; nativo > traduzido-por-léxico; nunca vazio). 'manual' nunca é sobrescrito; a tradução LLM (`garantirFichaPT`) corre FORA da fusão e sobrevive a re-fusões (o valor 'anterior' concorre). Idempotência provada no backfill (ronda final: 0 mudanças em 206 fichas).
