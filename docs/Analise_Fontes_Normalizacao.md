...
| D2 | **Folha como categoria de exibição** — SUPERADA pela REGRA do dono (2026-06-13): a seção do usuário é a ÚLTIMA subcategoria SEMPRE que o voto é fiável (`lista_item.cat_exib` em runtime), sem curadoria por tipo; os tipos à mão viram fallback. Guardas de casos reais: folha rasa (label de corredor) não exibe; ES não exibe; item com EAN só aceita via-EAN; anti-colisão por tokens. | ✅ v0.0.143.x |
