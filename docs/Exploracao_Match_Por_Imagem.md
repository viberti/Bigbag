...
## Corpo que cresce sozinho + gémeo no scan (2026-06-16)

A galeria-referência deixa de ser só o bulk de catálogo: as buscas reais **alimentam o Qdrant** e o gémeo já se calcula no scan (falta só mostrá-lo).

- **Persistência on-demand dos vetores** (`upsertVetores`, `matchImagem.js`): quando o `acharGemeo` (`resolverPorNome.js`) baixa+vetoriza candidatos OFF, **persiste TODOS os que vetorizou** (não só os que casaram) → o corpo cresce das buscas reais e amortiza o download lento do OFF (da 2.ª vez já cá está). Fire-and-forget. Checagem "já no corpo" por **payload.ean** (`eansNoQdrant`, scroll com filtro), independente do esquema de id.
- **id do ponto = `uuidDoEan(ean)` (UUIDv5), NÃO `Number(ean)`** — lição: os EANs-lixo curtos do OFF (ex.: `000000000054`→54) colidiam com os ids inteiros das linhas de catálogo na MESMA coleção (um teste chegou a apagar um ponto real). UUIDv5 determinístico do EAN é à prova de colisão e idempotente; o Qdrant aceita ids inteiros (catálogo) e UUID na mesma coleção.
- **Fila de vetorização em fundo — "falha hoje, acerta amanhã"** (migração 065 `fila_vetorizar`, ean PK; `ingest/filaVetor.js`; worker no `server.js`): quando o download SÍNCRONO da imagem OFF dá timeout no `acharGemeo` (teto de latência do scan ~6s), em vez de desistir **enfileira-se** `{ean, imagem_url}`. Um worker (`iniciarWorkerFila`, setInterval ~3min, lote 5, timeout largo 20s, `unref`) baixa→vetoriza→upsert com calma (até 5 tentativas) → da próxima vez já lá está. Resolve o ovo-galinha do download lento sem alargar a latência do scan.
- **Adoção de gémeo lê off_full E catálogo:** `adotarNomeRef` e a resolução de imagem do `/info` passaram a varrer o `off_full` **E** o `catalogo_produto`. Antes só liam o off_full → um gémeo presente apenas no NOSSO catálogo (ex.: BR) dava `null` e a adoção falhava **em silêncio**; agora adota-se na mesma.
- **Estado do gémeo no scan — calculado, ainda não mostrado (Fase 2 pendente):** o `/identificar` já devolve `gemeo` (convergência foto-CLIP + nome/marca do VLM) quando a ficha fica magra, mas o frontend **DESCARTA-o** (o `registar()` usa só o nome). Falta surfacar o cartão "a foto confirma — usar nutrição/foto deste código?". Nota: o `acharGemeo` só corre quando **NÃO há nutrição** (`!nutricao`) — se a foto do verso já deu a nutrição, nem dispara.
