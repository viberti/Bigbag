...
- **`GET /info`** (`item_id` **ou** `ean`) — `consolidarProduto`: consolida TUDO o que sabemos do produto (funde as linhas de `produto_ean`, genérico dos frescos, fotos; EAN do talão é autoritativo). Devolve ainda **`marca`/`marca_via`** (marca resolvida + proveniência), **`analise_ean`** (país/empresa/validade via `normaliza/ean.js`), **`ficha_magra`** (ficha sem nutrição nem imagem → a UI oferece adotar gémeo) e `sugestao_nome` (gémeo achado sob outro EAN).
