...
### 1d. Tabelas e colunas posteriores (migrações 033–059; DDL nas próprias migrações)
...
- **`acharPorNomeMarca(pool,{nome,marca,tamanho})`** (`normaliza/resolverPorNome.js`, PROVADO mas POR LIGAR ao fluxo) — EAN desconhecido fotografado → VLM lê nome+marca → busca o **mesmo produto sob OUTRO EAN** no `off_full` (FULLTEXT `ft_nome`; marca exigida = gate forte; tamanho desempata) → candidatos com nutrição+`imagem_url`. A ligar: cruzar com o match-por-imagem (`matchImagem.js`) + fusão de confiança + confirmação + adotar/vetorizar.
