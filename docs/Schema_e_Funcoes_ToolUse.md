...
- **`lista_item` / `lista_pessoal` (034):** lista de compras PARTILHADA da família (servidor é fonte de verdade); estados `ativo|carrinho|comprado|removido`, `adicionado_por`/`marcado_por` (cor do membro), `fatura_id` (reconciliação). `lista_pessoal` = listas individuais (UNIQUE user+nome). **Lista inteligente fase 1 (2026-06-11, sem colunas novas — tudo derivado do histórico em runtime):** `GET /api/lista` devolve por item `produto_sugerido` (variante mais comprada pela casa), `variantes_n` e `qtd_habitual` (unidades/ida); `GET /api/lista/variantes?nome=` lista as variantes habituais (idas, qtd habitual, preço €/base + loja) para o seletor; `PATCH /api/lista/:id` aceita `nome` (escolher variante = concretizar o item). Preço do item = `preco_por_base` (fallback `preco_unitario` quando não há ppb). Determinístico, zero LLM.
