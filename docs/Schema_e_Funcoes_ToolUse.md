...
### 1d. Tabelas e colunas posteriores (migrações 033–068; DDL nas próprias migrações)
...
- **Migrações 053–057 (match-por-imagem + EANs + histórico):** 053 `catalogo_produto.foto_em/vetor_em` (tracking do pipeline match-por-imagem: foto baixada p/ disco em `/var/lib/bigbag/imagens/{id}.jpg` + vetor no Qdrant; bulk reentrante/mensurável); 054 **índice `catalogo_produto.marca`** (a lista/despensa fazem `WHERE marca = ?`/`IN (...)`; eram full-scans de ~62k linhas → o grosso dos 6-7s da despensa); 055 **vista `v_ean_universo`** (mapa/denominador do UNIVERSO de EANs distintos das fontes c/ flags de presença por área — analítico/vetorização, NÃO lookup quente: para resolver um EAN usar `fundirFichaEan`); 056 **`catalogo_match`** (elos CROSS-LOJA por imagem+metadados: liga produto SEM EAN — PD/Lidl — ao mesmo COM EAN; camada de sugestões/proveniência, 1 linha/origem, reentrante; distinta de `match_ean_sugestao` que liga descrições de talão); 057 **`historico_produto`** (produtos CONSULTADOS pelo utilizador — toda ficha aberta; `chave` dedup `e:<ean>·s:<sku_id>·n:<nome>` com `n_consultas`/`ultima_em`; alimenta a tela Histórico da v2).
- **`catalogo_produto.product_type` (058) + `normaliza/tipoProduto.js` — ALIMENTO vs NÃO-ALIMENTO (2026-06-14):** classificação persistente `'food'|'non_food'|NULL`. `tipoProduto({nome,temNutricao,foodGroups,categoria})` é **puro+testado** (`test/tipoProduto.test.mjs`, gate), de-acentua a entrada (o `\b` do JS falha junto a acentos) e decide por ordem: tem nutrição/food_groups → **food**; padrões de NÃO-ALIMENTO no NOME (fiáveis mesmo com categoria errada) → non_food; categoria de loja/OFF de não-alimento (PT+ES p/ Mercadona) → non_food; alimento-sem-nutrição pelo nome (água/vinho/especiarias) → food; categoria de alimento → food; senão NULL. Backfill determinístico `scripts/backfill_product_type.mjs` (~65%: food 40.300, non_food 14.414, NULL 29.742 em 84.456) + os ambíguos por LLM `scripts/classificar_ambiguos_llm.mjs` (gemini-2.5-flash, lotes configuráveis, cursor por id, contexto `classificar_tipo`, ~$1 total). Consumidores: `consolidarProduto` (`routes/produto.js`) devolve `tipo` (preferindo o `product_type` gravado, fallback live `tipoProduto`) + `catalogo_categoria`; a ficha v2 esconde Para-Sue/nutrição em não-alimentos.
- **`off_full` — OFF COMPLETO LOCAL (2026-06-15, sem migração numerada — criada por script):** ~**4,5M** produtos do dump CSV do Open Food Facts, importados por **DuckDB** (`scripts/` no PC + `/home/dev/offdump`). Colunas: `ean` (PK), `nome`, `marca`, `quantidade`, `categoria`, `categorias_tags`, `paises_tags`, `ingredientes`, `alergenios`, `nutriscore`, `nova`, **`imagem_url`/`imagem_small`** (3,3M têm), e **nutrição em colunas planas** (`energia_kcal`, `gordura`, `gordura_sat`, `hidratos`, `acucares`, `proteinas`, `sal`, `fibra`; 2,2M têm). **`consultarOFF` (`ingest/produto.js`) e a fusão (`fichaEan.js`) leem o `off_full` como BACKSTOP** do `off_produto` (nutrição+imagem por EAN) ANTES da API live → corta milhões de chamadas. Colação `utf8mb4_0900_ai_ci` (joins por ean precisam `COLLATE`). FULLTEXT `ft_nome(nome,marca)` adicionado (resolução por nome de EAN desconhecido — ver `acharPorNomeMarca`). **NÃO é superfície de busca por nome PT** (multilíngue, recall baixa); para isso → `produto_busca`. *Bug corrigido (v0.0.180.0): um `off_json.nutricao_100g={}` vazio bloqueava o off_full bom — `offLive` só vence se `temNut`.*
- **`produto_busca` (migração 059) — índice de BUSCA por nome (2026-06-15):** materializado (~54k) do catálogo PT-comprável (lojas PT + Mercadona-ES/Lidl-FR traduzidos, dedup por EAN/nome) por `scripts/construir_produto_busca.mjs` (TRUNCATE+rebuild; idempotente). Colunas: `generico` (1=genérico, topo), `nome`, `marca`, `tamanho`, `ean`, `product_type`, `popularidade` (vezes listado/comprado pela casa), `tem_nutricao`; **FULLTEXT `ft_nome(nome,marca)`**. **Genéricos** = substantivo-cabeça por frequência (`singularizar` da 1.ª palavra) + termos do `lista_item` (histórico da casa). Consumido pelo `GET /api/produto/autocomplete` (ver §2b). Settings globais do FULLTEXT NÃO tocadas (`min_token_size=3`, MySQL partilhado com pitacos.ai/1417).
...
- **Migrações 062–066 (locale por utilizador + sinais de EAN + fila de vetores + perfil de saúde):**
  - **`usuario` (062):** camada PREÇO+LOCALE por país (`Visao_Multi_Pais`). `email` PK, `pais` CHAR(2) default `'PT'`, `locale` VARCHAR(5) default `'pt-BR'`. O país decide moeda + fontes de preço; a IDENTIDADE (por EAN) continua partilhada. Resolve-se por email na auth. Semente idempotente dos dois utilizadores conhecidos. Alterável por `POST /api/me/pais`.
  - **`ean_empresa` (063):** prefixo GS1 de empresa → **marca dominante minada por prefixo** (`Classificacao_Fusor §5.4`). `prefixo` (8 dígitos) PK, `marca`, `pais` (GS1, via `paisDoEan`), `n_produtos`, `share` DECIMAL(4,3) = **pureza/coerência** do prefixo = peso do voto (alto → empresa-especialista, rotear com confiança; baixo → prefixo partilhado, não asserir marca). Minado dos nossos `(ean,marca)`; reconstruível.
  - **`catalogo_produto` FULLTEXT `ft_nome_marca(nome,marca)` (064):** a busca de **gémeo por texto** (`acharGemeo`/`acharPorNomeMarca`) passa a varrer TODAS as nossas fontes (lojas PT/ES/BR, Nutripédia, harvests) ALÉM do `off_full` — apanha gémeos que só existem no nosso catálogo. `min_token_size=3` global (partilhado) não tocado.
  - **`fila_vetorizar` (065):** fila de **vetorização em fundo** ("falha hoje, acerta amanhã"). Quando o download da imagem de um candidato OFF dá timeout no caminho síncrono do `acharGemeo`, enfileira-se aqui; um worker (`ingest/filaVetor.js`) baixa com calma, vetoriza e faz upsert no Qdrant. `ean` PK, `imagem_url`, `fonte`, `tentativas`, `ultimo_erro`, `criado_em`, `processado_em`; índice `ix_pendente(processado_em,tentativas,criado_em)`. Idempotente.
  - **`perfil_membro.saude_estado` JSON (066):** estado do **editor de perfil de saúde** (handoff cartoon v13): `{ inativas:{...}, demografia:{email,idade,sexo,peso,altura} }` (características desativadas + demografia do membro). Fica à parte para o `resumo` ficar **PURO** — só as características ATIVAS, que `avaliarParaPerfil` serializa inteiro no prompt.
- **`base_local` + `base_local_evento` (migrações 067-068) — BASE LOCAL no telefone, medida e que CRESCE (2026-06-17):** réplica de **identificação+nutrição** no IndexedDB para o scan responder **instantâneo/offline**, agora também na **v2** (antes só a v1 usava base local). Decisão do dono: *"começar com menos, medir a taxa de hits, ajustar"*.
  - **`base_local`** (EAN PK): `nome`, `marca`, `quantidade`, `categoria`, `product_type`, `alergenios`, `nutriscore`, `nova`, `nutricao` JSON (por 100g, shape `{sal,fibra,gordura,acucares,hidratos,proteina,energia_kcal,gordura_saturada}`), `ingredientes`, `origem` (`pt_cat`|`merc_es`|`pt_off`|**`uso`** = crescido com o uso, nome PT-fiável | **`uso_es`** = crescido mas nome ainda estrangeiro, fora do atalho da lista), `atualizado_em`, **`seq` BIGINT AUTO_INCREMENT UNIQUE (068)**. **Bootstrap** por `scripts/build_base_local.mjs` (fusão **offline** sem LLM/live: catálogo PT `auchan/continente/pingodoce/nutripedia` + Mercadona ES + EANs PT do `off_full`; nutrição catálogo>off_full): **~63k fichas** (46% com nutrição), ~37 MB no telefone. Build **idempotente por upsert (sem TRUNCATE)** → re-correr preserva as linhas `uso`. LIDL/ALDI ficam **de fora de propósito** (medir o miss antes de incluir).
...
Resumo das rotas montadas em `backend/src/routes/`. Todas exigem sessão. **`requireAuth` (`backend/src/auth.js`, 2026-06-14)** aceita, por ordem: (1) **Bearer JWT do Zitadel** — valida assinatura via JWKS + issuer (`jose`, `createRemoteJWKSet`/`jwtVerify`), obtém o email (token ou `/oidc/v1/userinfo` cacheado) e exige-o na **allowlist** (`config.auth.allowlist` ← `AUTH_ALLOWLIST`); **403** se autenticado mas fora da allowlist; (2) **HTTP Basic** (`ENABLE_TEST_AUTH`/`TEST_USERS`) de fallback durante a migração. `config.auth.oidcIssuer` (`OIDC_ISSUER`, default `https://auth.hal9klabs.com`). **NÃO envia `WWW-Authenticate: Basic` no 401** (senão o browser abria o diálogo nativo de Basic e tapava o login OIDC da app). **Login Google a funcionar** (utilizador regista-se sozinho; allowlist por email). Detalhe completo do IdP em `docs/Auth_Zitadel.md`.

**Sessão / utilizador (`server.js`, top-level):**
- **`GET /api/me`** — devolve `{ user }` com email + **`nome`** (primeiro nome legível das claims OIDC: `given_name` do Google, senão a 1.ª palavra de `name`; `null` → o frontend cai no email).
- **`POST /api/me/pais`** — atualiza o `pais` do `usuario` (062) e invalida o cache da auth (o próximo `/api/me` já reflete).

**Busca por nome (2026-06-15):**
- **`GET /api/produto/autocomplete?q=&modo=lista|nutricao`** — autocomplete sobre `produto_busca` (FULLTEXT, ~54k PT-comprável). Tokens: último=prefixo (`+x*`), anteriores=palavra exata (`+x`). Ranking: palavra-exata(+100) > começa-com(+40), **genérico no topo**(+1000), alimento 1.º na lista(+30), `popularidade` desempata → mata os "esquisitos" do prefixo (leite≠leiteira). Devolve `{sugestoes:[{generico,nome,marca,tamanho,ean,tem_nutricao}]}`, ~4-17ms. Frontend: dropdown no campo "escrever" da lista (genéricos em negrito).
- **`acharPorNomeMarca(pool,{nome,marca,tamanho,termos})`** (`normaliza/resolverPorNome.js`, **LIGADA** — usada por `consolidarProduto` e por `acharGemeo`) — EAN desconhecido → busca o **mesmo produto sob OUTRO EAN** no `off_full` E no nosso `catalogo_produto` (FULLTEXT `ft_nome`/`ft_nome_marca` 064; marca = gate forte via `nomeCondizGemeo`; tamanho desempata) → candidatos com nutrição+`imagem_url`. `acharGemeo` cruza ainda com o match-por-imagem (`matchImagem.js`) e enfileira a vetorização em fundo (`fila_vetorizar` 065) quando o download da imagem demora.
...
- **`GET /info`** (`item_id` **ou** `ean`) — `consolidarProduto`: consolida TUDO o que sabemos do produto (funde as linhas de `produto_ean`, genérico dos frescos, fotos; EAN do talão é autoritativo). Devolve ainda **`marca`/`marca_via`** (marca resolvida + proveniência), **`analise_ean`** (país/empresa/validade via `normaliza/ean.js`), **`ficha_magra`** (ficha sem nutrição nem imagem → a UI oferece adotar gémeo) e **`sugestao_nome`** (gémeo achado sob outro EAN). **Gate da sugestão a 3 sinais (2026-06-17):** (1) `nomeCondizGemeo` (nome/termos condizem, não só a marca); (2) **família tem de bater** (`familiaPorNome`; só rejeita quando ambas conhecidas e diferem — Páprica/especiarias ≠ Milho/conservas) e **nunca em não-alimento** (`tipo!=='non_food'`); (3) **`confirmarGemeoPorImagem`** (CLIP) — a sugestão ganha **`confirmada_imagem`** (`score≥0.72`) comparando a foto do utilizador (`produto_foto`) com a imagem do candidato; a UI só mostra a **foto** do candidato com `confirmada_imagem` (sem foto de referência → escondida).
...
- **BASE LOCAL (telefone, migrações 067-068):**
  - **`GET /base-local-fichas`** (`?desde_seq=`) — serve a `base_local` (~63k EANs PT+Mercadona-ES com nutrição) **incremental por cursor de `seq`** (apanha o crescimento vivo) + `versao`. O telefone (`baseLocal.js → sincronizarFichasBulk`) escreve na store `fichas` do IndexedDB; o scan resolve **local-first** (instantâneo/offline).
  - **`POST /local-hit`** (`{ean, hit, origem}`) — telemetria: o telefone diz se RESOLVEU o EAN localmente (hit) ou foi ao servidor (miss) → `base_local_evento`.
  - **`GET /base-local`** (legado v1) — fichas resolvidas (`produto_ean`+análise, completas) + catálogo nome→EAN incremental por id. Usado pela v1 (App.jsx, congelada); a v2 usa o `base-local-fichas` acima.
  - *Crescimento:* `consultarOuGuardar` faz `upsertBaseLocal` (origem `uso`) a cada EAN resolvido no servidor → entra na base partilhada de todos.

### Perfil nutricional — `/api/perfil` (`perfil.js`)
- **`GET /`** — lista perfis, o ativo primeiro; por perfil devolve **`resumo`** (características ativas) + **`saude_estado`** (inativas + demografia, 066).
- **`POST /`** (`{ nome, texto, demografia? }`) — carrega/atualiza um perfil: extrai o `resumo` estruturado (LLM) e fica **ativo** (upsert por nome; ativa só este); a **`demografia`** (email/idade/sexo/peso/altura) entra em `saude_estado` sem poluir o `resumo`.
- **`POST /:id/ativar`** — torna esse perfil o ativo (`ativo = IF(id=?,1,0)`).
- **`POST /:id/saude`** (`{ ativas, inativas, demografia, notas }`) — editor de saúde (066): só as **ativas** entram no `resumo` (que a avaliação lê); **inativas** + **demografia** ficam em `saude_estado`; não mexe no `texto` original.
...
- **Base local** (cobertura do telefone, 067-068): `GET /base-local?dias=` — `{ resumo (taxa hit telefone vs servidor), base (tamanho por origem), porOrigem, porDia, misses }`. Os **top misses** vêm cruzados com `off_full` (nome/marca/país) = candidatos a adicionar à base (p.ex. LIDL/ALDI). Aba "Base local" no `/admin`.

### Funções de apoio (ingestão)
- **`ingest/produto.js`:** `extrairProdutoFotos` (VLM sobre N fotos, **com RETRY 2× em JSON malformado**), `consultarOFF` (Open Food Facts por EAN), `analisarProduto` (análise factual), `caracterizarProdutoNome` (fresco vs. processado + nutrição típica), `sugerirNomeCanonico` (melhor nome PT genérico das variantes), `eanValido` (check-digit GTIN-8/12/13), `lerEanDeFoto`, `analisarFotoProduto` (classifica talão/produto/outro), `buscarOffPorNome`, **`arbitrarMarcaNome({off,vlm,fotos})`** — árbitro **multimodal** de nome/marca (modelo de extração, lê as fotos + julga OFF↔VLM); só é chamado **no conflito** OFF↔VLM (em `consolidarProduto`).
- **`normaliza/ean.js` (sinais do EAN, puro):** `eanValido` (check-digit), `eanInterno` (EAN de loja/interno, sem GS1), `paisDoEan` (país GS1 pelo prefixo), `analiseEan(ean,{empresa})` (objeto consolidado país/empresa/validade — exposto pelo `/info` como `analise_ean`).
- **`ingest/filaVetor.js` (fila de vetorização, 065):** `enfileirar(pool,itens)`, `processarFila(pool,{lote})`, `iniciarWorkerFila(pool,{intervaloMs,lote})` — worker em fundo que esvazia `fila_vetorizar` (baixa imagem → vetoriza → upsert no Qdrant).
- **`normaliza/matchImagem.js` (match-por-imagem / Qdrant):** `vetorizarVariasB64(b64s)` (CLIP em lote), `cosseno(a,b)`, `uuidDoEan(ean)` (id determinístico do ponto), `upsertVetores(pontos)`, `eansNoQdrant(eans)` (quais já estão vetorizados), + `matchImagemB64`/`matchPorVetor`/`estadoMatchImagem`.
- **`normaliza/resolverPorNome.js` (gémeo por nome):** `acharGemeo(pool,{fotoB64,nome,marca,tamanho,termos,tipoTexto,eanProprio})` — orquestra texto (catálogo+`off_full` via `ft_nome_marca`/`ft_nome`) **e** imagem; `nomeCondizGemeo({nome,marca,termos,candNome})` (gate de coerência do candidato); `acharPorNomeMarca` (busca textual pura — ver §2b). A **sugestão da ficha** (`consolidarProduto`) acrescenta a estes o gate de **família** (`familiaPorNome`) + **não-alimento** e a **confirmação por imagem** `confirmarGemeoPorImagem` (CLIP: foto do utilizador × imagem do candidato → `confirmada_imagem`).
- **`normaliza/categoria.js`:** `marcaEhTipo(marca)` — marca que é, na prática, um tipo de produto (entra na decisão de grupo/exibição).
- **`ingest/extract.js`:** `parseJsonLoose` agora tolera **vírgulas finais** (trailing commas) no JSON do VLM/LLM antes de desistir.
- **`ingest/perfil.js`:** `perfilParaTexto(resumo)` — serializa o resumo do perfil num **bloco etiquetado** (em vez de JSON cru) para o prompt de `avaliarParaPerfil`/`compararProdutosLLM`.
- **`ingest/perfil.js`:** `extrairPerfil` (texto → resumo estruturado), `alertasDoPerfil` (determinístico; grupos de sinónimos de alergénios PT↔EN/OFF, limpa prefixos `en:`/`pt:`), `avaliarParaPerfil` (parecer do LLM, factual não clínico), `perfilParaTexto` (resumo → bloco etiquetado p/ o prompt).
