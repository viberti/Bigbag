...
- **2.2 Higiene da marca — FEITO (2026-06-13).** 458 anuladas (#ref! ×443 + numéricas puras), 68 marcas RECUPERADAS do sufixo de parsing francês do lidl-fr ("Formil Bouteille, Flacon…"→"Formil"; "Sondey Non 93,00%…"→"Sondey"); compostas Mercadona ("Hacendado, Schreiber") preservadas DE PROPÓSITO (co-branding legítimo; no gazetteer só casam com ambas as partes). Lixo restante: 0.
- **3.1 Categoria de loja como sinal — FEITO (2026-06-13, v0.0.132.0).** Medição: `grupoDeTexto` já resolvia 78–95% dos paths PT (Mercadona ES 53%). Vocabulário ganhou termos com evidência nos paths (ES: cerveza/vino/bodega/queso/jamon/panaderia/bolleria/champu/maquillaje/aceituna…; PT: espumante/nectar_/nectare/peixaria/infus/azeitona). Integração: a lista usa `categoria_path` do catálogo quando o nome dá 'outros' (`aplicarDadosEan`). Resultado vivo: itens sem-grupo 10→5 (infusões→bebidas, Polpa de Tomate/Palmito→mercearia); os 5 restantes não estão em catálogo nenhum (honesto). Fora-do-domínio (Mascotas/Livros/Jardim) fica 'outros' DE PROPÓSITO. **O golden apanhou a 1.ª regressão real em 30s:** 'nectar' casava "Nectarina"→bebidas — corrigido (`nectar_`+`nectare`) antes de sair do PC.
- **3.2 Sinónimos no matching — FEITO (2026-06-13, v0.0.133.0).** Mineração dos 3.565 pares mesmo-EAN entre fontes rendeu POUCO (catálogos PT escrevem parecido; o alinhamento 1-a-1 dá sobretudo peso-vs-palavra) — documentado. Implementado `SINONIMOS` curado em `tokenCasa` (esparguete|spaghetti, yogur|iogurte, queso|queijo, mozzarella|mozarela, hummus, tampones — casos reais + minados genuínos; chaves PÓS-singularização). Só no matching, NÃO na chave de consolidação. Crescer com casos reais, não em massa.
- **3.3 Fuzzy — MEDIDO E REJEITADO (2026-06-13).** 17,5% dos tokens raros estão a ed-1 de um comum, MAS a amostra mostra que são plurais legítimos e TAMANHOS ("405ml~400ml", "6x60g~6x50g") — fuzzy genérico criaria falsos positivos perigosos; typos reais (regarga, cookes, vinagrette) são raros. Fica rejeitado com critério: reabrir só se a taxa de não-resolvidos subir E os casos forem typos comprovados.
- **3.4 Auditoria do scan — FEITO (2026-06-13).** `auditar_grupos.mjs --scan` audita os nomes vivos de lista+despensa com o MESMO juiz calibrado por canários (1.ª corrida: 5/5 canários, 8 flags → 4 termos claros adotados: picles/pickles/passata/palmito; ambíguos recusados de propósito). **Bónus:** o prompt do juiz julgava com a taxonomia de ANTEONTEM (padaria=cereais+massas) — atualizado. Cadência: mensal ou após sessão grande de scans (regra no CLAUDE.md).
- **3.6 Plausibilidade da nutrição — FEITO (2026-06-13).** `nutricaoPlausivel` em validadores.js (kcal≤950, macros 0–100, açúcar≤hidratos, saturada≤gordura, soma≤105); nutrição SÓ-VLM implausível é descartada com warn em vez de entrar na ficha. 9 testes.
- **3.7 Miudezas — FEITAS (2026-06-13).** `cortarGenerico` extraído para o módulo partilhado COM testes (compostos protegidos); marcas curtas reais no gazetteer com evidência (W5 ×403, Bic, Cif, Axe, Uhu, Tuc, Ogx, Xau — allowlist; matching por token exato é seguro); prompt da canonicalização ganhou a frase dos conectores; `candidatosOFF` loga erros HTTP (vigiar 429 antes de precisar de rate-limiter).
- **3.5 Vocabulário em BD — NÃO FEITO de propósito** (marcado opcional; código+golden chegam à escala atual; reavaliar se o whack-a-mole de termos incomodar).
- **2.1 Unificação front/back — FEITO (2026-06-13, v0.0.131.0).** `normaliza/categoria.js` é o módulo PARTILHADO (puro, zero imports; o frontend importa-o por path relativo — Vite resolve): exporta `norm`/`normAlfa` + `TIPOS_NOME`/`tipoConsumidor`/`GEN_RE`/`CONECTORES`. `norm()`: 5 cópias substituídas por import (lista, produto, marca, resolverProduto, reconciliarLista); `facetas.js` e `verificarNomes.js` ficam próprias DE PROPÓSITO (documentado no cabeçalho). App.jsx perdeu as 4 cópias (import com alias = zero mudança de usos). **Ganho comprovado:** `chaveLite`→`chaveItemLista` — a consolidação otimista do telemóvel usa a MESMA chave do servidor ("Pão"+"Pães" agora consolidam no otimista; antes só no servidor → flash de duplicado). Equivalência verificada: tipoConsumidor 417/417 nomes iguais; golden 325/325; suite 180/180. Fora do âmbito desta passada (anotado): `queries.js normaliza`, `Explorar.jsx norm`, fusão de TERMOS entre TIPOS_NOME e GRUPOS (fica para 3.x).
