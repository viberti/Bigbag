...
> Sobram dois problemas **separados do cálculo**: (1) **má nutrição na fonte** que é "possível mas errada" (ex.: olive oil com saturada 72 % — palm/coco existem, logo não dá para rejeitar genericamente); (2) **classificação**: "Croutons/Petinga **com/em azeite**" entram como família azeite e ganham nota inflada → ver tarefa de classificação separada em `familiaPorNome`.
