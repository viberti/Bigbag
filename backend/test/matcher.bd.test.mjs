...
test('descrição nova → cria SKU (via novo) e grava alias', async () => {
  const r = await resolverSku(conn, 'MANTEIGA ZZTEST 250G', { canonicalizar: stub });
  assert.equal(r.via, 'novo');
  assert.ok(r.sku_id > 0);
});

test('mesma descrição → resolve por alias (cache), mesmo sku_id', async () => {
  const r1 = await resolverSku(conn, 'MANTEIGA ZZTEST 250G', { canonicalizar: stub });
  const r2 = await resolverSku(conn, 'MANTEIGA ZZTEST 250G', { canonicalizar: stub });
...
test('descrição diferente, mesmo produto → emparelha ao SKU existente (via match)', async () => {
  const r1 = await resolverSku(conn, 'MANTEIGA ZZTEST 250G', { canonicalizar: stub });
  const r3 = await resolverSku(conn, 'MANT ZZTEST 250G', { canonicalizar: stub });
...
test('Camada 3: variante do mesmo produto agrupa por similaridade', async () => {
  ...
  const r1 = await resolverSku(conn, 'PARMIGIANO ZZTEST', { canonicalizar: stubVar });
  const r2 = await resolverSku(conn, 'PARMIGIANO ZZTEST DOP 24M', { canonicalizar: stubVar });
