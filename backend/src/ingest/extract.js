...
Esquema exato:
{
  ...
  "nif_comprador": string|null,     // NIF do CLIENTE/consumidor SE impresso ("Contribuinte", "NIF do adquirente", "NIF Cliente" — normalmente junto ao total ou no fim). NÃO é o NIF da loja (esse vai em loja.nif). null se a compra não tem NIF de cliente.
  "forma_pagamento": string|null,   // como foi pago, do rodapé: "dinheiro" | "cartao" | "mbway" | "outro". Rótulos: Numerário/Dinheiro→dinheiro; Multibanco/Cartão/Débito/Crédito/TPA/CARTAO BANCARIO→cartao; MB WAY→mbway. null se não aparecer.
  "itens": [
    {
...
- DATA DA COMPRA ("data_compra"): usa a data do DOCUMENTO — rótulos "Data de Venda", "Data de emissão", "Data". CUIDADO com a linha do TERMINAL de pagamento (Multibanco/SIBS), que imprime **AA-MM-DD** (ano com 2 dígitos PRIMEIRO, ex.: "26-08-07" = 7 de agosto de 2026) — NÃO a leias como DD-MM-AA. Se a única data visível estiver CORTADA, desfocada ou ilegível (sobretudo o ANO), devolve **null** — NÃO inventes nem adivinhes o ano. Um null é tratado (assume-se a data da captura); um ano errado arquiva a compra anos no passado e desaparece do histórico.
- "Aprox. fim prazo validade" aparece NA LINHA ABAIXO do produto — associa ao item imediatamente acima (is_clearance=true).
- CABEÇALHOS DE SECÇÃO NÃO são parte do nome: alguns talões (Pingo Doce) imprimem o nome da SECÇÃO da loja (MERCEARIA, MERCEARIA + PET FOOD, CONGELADOS, TAKE-AWAY, PADARIA, TALHO, PEIXARIA, FRUTAS E VEGETAIS, LACTICÍNIOS, BEBIDAS, HIGIENE…) numa linha própria ANTES dos itens dessa secção. NUNCA coles esse cabeçalho ao nome do item seguinte.
