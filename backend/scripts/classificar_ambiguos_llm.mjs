...
async function classificar(rows) { // → mapa {índice: 'food'|'non_food'} (ou null)
  ...
  try {
    ...
    return parse(out);
  } catch (e) { console.error('  chamada falhou:', e.message); return null; }
}

// CURSOR por id: avança mesmo nos lotes falhados (que ficam NULL e são re-tentados
// numa próxima corrida desde o cursor 0) — nunca fica preso no mesmo lote mau.
let lotes = 0, total = 0, food = 0, non = 0, falhou = 0, cursor = 0;
for (;;) {
  const [rows] = await pool.query(
    `SELECT id, COALESCE(nome_pt, nome) AS nome, categoria FROM catalogo_produto
      WHERE product_type IS NULL AND id > ? AND COALESCE(nome_pt, nome) IS NOT NULL AND COALESCE(nome_pt, nome) <> ''
      ORDER BY id LIMIT ?`, [cursor, LOTE]);
  if (!rows.length) break;
  let mapa = await classificar(rows);
  if (!mapa) mapa = await classificar(rows); // 1 retry (hiccup/JSON malformado)
  const foodIds = [], nonIds = [];
  rows.forEach((r, i) => {
    const v = mapa?.[String(i + 1)];
    if (v === 'food') foodIds.push(r.id);
    else if (v === 'non_food') nonIds.push(r.id);
    else falhou++; // sem resposta válida → fica NULL (re-tentável)
  });
  ...
  cursor = rows[rows.length - 1].id; // avança SEMPRE (mesmo se o lote falhou)
  food += foodIds.length; non += nonIds.length; total += rows.length; lotes++;
  if (lotes % 10 === 0 || (LIMITE && lotes >= LIMITE)) console.error(`  lote ${lotes}: acum food ${food}, non ${non}, s/resposta ${falhou} · cursor ${cursor}`);
