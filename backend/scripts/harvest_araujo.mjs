// ADAPTADOR Araújo (araujo.com.br) — grande rede mineira (MG). Server-rendered e limpa: a
...
async function precoDoEan(ean) {
  const r = await precoAraujoEan(ean);
...
async function alvos(pool, { limite, atualizar, eans }) {
  ...
  const cond = atualizar ? '' : `AND NOT EXISTS (SELECT 1 FROM catalogo_produto ar WHERE ar.ean=m.ean AND ar.fonte='${FONTE}')`;
